...
import com.wangtiansoft.KingDarts.common.utils.ZxingKit;
import com.wangtiansoft.KingDarts.common.utils.wxpay.WXPayUtil;
...
//	static Log log=Log.getLog(WeixinPayController.class);
	 ...
	 */
	@RequestMapping("/list")
	public @ResponseBody ApiConfig getApiConfig() {
	...
	@RequestMapping("")
	public @ResponseBody Map<String, String> index(String openId,String total_fee) {
		...
		params.put("mch_id", PropKit.get("mch_id"));
		...
		params.put("notify_url", PropKit.get("domain")+"/pay/pay_notify");
		params.put("openid", openId);

		String sign = PaymentKit.createSign(params, PropKit.get("paternerKey"));
		...
		String packageSign = PaymentKit.createSign(packageParams, PropKit.get("paternerKey"));
	...
	public @ResponseBody String pay_notify() {
		//获取所有的参数
...
//		Map<String, String> params = PaymentKit.xmlToMap(xmlMsg);
		...
		try {
			params = WXPayUtil.xmlToMap(xmlMsg);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		...
		//		if (order==null) {
		if(PaymentKit.verifyNotify(params, PropKit.get("paternerKey"))){
	...
	public String getCodeUrl(){
		...
		Map<String, String> packageParams = new HashMap<String, String>();
		packageParams.put("appid", PropKit.get("appId"));
		packageParams.put("mch_id", PropKit.get("mch_id"));
		packageParams.put("product_id",product_id);
		packageParams.put("time_stamp", timeStamp);
		packageParams.put("nonce_str", nonceStr);
		String packageSign = PaymentKit.createSign(packageParams, PropKit.get("paternerKey"));

		return StringUtils.replace(url, "XXXXX", packageSign,PropKit.get("appId"),PropKit.get("mch_id"),product_id,timeStamp,nonceStr);
	}
	...
	public String scanCode1(){
		...
		if (encode) {
			//在页面上显示
			request.setAttribute("payQRCode", name);
...
//		renderText(getCodeUrl());
	...
	public @ResponseBody String wxpay(){
		try {
//			HttpServletRequest request = getRequest();
			...
			String paysign = PaymentKit.createSign(params, PropKit.get("paternerKey"));
			...
			if (StrKit.notBlank(return_code) && StrKit.notBlank(result_code) && return_code.equalsIgnoreCase("SUCCESS")&&result_code.equalsIgnoreCase("SUCCESS")) {
				...
				prepaySign = PaymentKit.createSign(prepayParams, PropKit.get("paternerKey"));
...
//			renderText(xmlResult);
			request.setAttribute("error", xmlResult);
			return page;
		}
		String result_code = result.get("result_code");
		if (StrKit.isBlank(result_code) || !"SUCCESS".equals(result_code)) {
...
//			render("payQRCode.jsp");
	...
	public @ResponseBody String micropay(String auth_code){
...
//		renderText(xmlResult);
	...
	public @ResponseBody String appPay(){
		//不用设置授权目录域名
...
//			renderJson(ajax);
		...
		Map<String, String> packageParams = new HashMap<String, String>();
		packageParams.put("appid", PropKit.get("appId"));
		packageParams.put("partnerid", PropKit.get("mch_id"));
...
//		renderJson(jsonStr);
	...
	/*@PostMapping("/save")
	public @ResponseBody ApiResult save(final SigninResult signin) {
		ApiResult result = this.buildMobileAjaxResponse(new IWebResponseHandler() {
			@Override
			public Object execute() throws Exception {
				signinService.saveOrUpdateSignin(signin, getAuthUserId());
				return null;
			}
		});
		return result;

...
<#include "./a/commons/top.ftl" />
    <style>
        #fa-icons .col-md-3{margin: 8px 0;}
        #fa-icons .col-md-3:hover{cursor: pointer;}
    </style>
</head>
<#escape x as x?html>
...
<script type="text/javascript">
    $("document").ready(function () {
        //  设置页面标题
        WT.wt_set_navtitle('模块图标');
        //  关闭
        $(".wt-close").click(function () {
            WT.wt_close();
        });
        //  选择图标
        $("#fa-icons .col-md-3").click(function () {
            var text = $(this).find("i").attr("class").replace(/fa fa-fw /, '');
            parent.$("#icon_display").attr('class', 'fa fa-fw ' + text);
            parent.$("#icon").val(text);
            WT.wt_close();
        })
    });
</script>

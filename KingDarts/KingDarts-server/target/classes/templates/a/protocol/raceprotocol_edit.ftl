    ...
    <div class="wt-layer-footer">
        <div class="col-sm-12 wt-layer-footer-content wt-btns-center">
            <input type="hidden" name="id" id="id" value="${entity.id}"/>
             <@wtAuth hasPrem="KINGDARTS_RACEPROTOCOL:EDIT" ><button type="button" class="btn btn-sm btn-info wt-save">保存</button></@wtAuth>
        ...
        $(".wt-save").click(function () {
            ...
            if($("#id").val()!=''){
	            WT.wt_confirm('是否保存?', function () {
	                WT.wt_ajax_form('${base}/a/protocol/raceprotocol_edit','layerForm',function(data){
                ...
                WT.wt_confirm('是否保存?', function () {
	                WT.wt_ajax_form('${base}/a/protocol/raceprotocol_add','layerForm',function(data){
	                    WT.wt_alert('保存成功',function(){
	                        WT.wt_reload_jqtable(parent);
	                        WT.wt_close();
	                    });

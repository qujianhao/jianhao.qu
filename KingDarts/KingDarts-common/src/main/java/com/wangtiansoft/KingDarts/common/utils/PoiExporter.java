...
public class PoiExporter {
    ...
    public Workbook export() {
        ...
        Workbook wb;
        HSSFCellStyle hcellStyle = null;
        XSSFCellStyle xcellStyle = null;
        XSSFCellStyle xHeardcellStyle = null;
        if (VERSION_2003.equals(version)) {
            wb = new HSSFWorkbook();
            hcellStyle = (HSSFCellStyle)wb.createCellStyle();
            hcellStyle.setBorderBottom(HSSFCellStyle.BORDER_THIN); //下边框    
            hcellStyle.setBorderLeft(HSSFCellStyle.BORDER_THIN);//左边框    
            hcellStyle.setBorderTop(HSSFCellStyle.BORDER_THIN);//上边框    
            hcellStyle.setBorderRight(HSSFCellStyle.BORDER_THIN);//右边框    
            hcellStyle.setAlignment(HSSFCellStyle.ALIGN_CENTER); // 居中
            ...
            wb = new XSSFWorkbook();
            xHeardcellStyle = (XSSFCellStyle)wb.createCellStyle();
            xHeardcellStyle.setBorderBottom(HSSFCellStyle.BORDER_THIN); //下边框    
            xHeardcellStyle.setBorderLeft(HSSFCellStyle.BORDER_THIN);//左边框    
            xHeardcellStyle.setBorderTop(HSSFCellStyle.BORDER_THIN);//上边框    
            xHeardcellStyle.setBorderRight(HSSFCellStyle.BORDER_THIN);//右边框    
            xHeardcellStyle.setAlignment(HSSFCellStyle.ALIGN_CENTER); // 居中
            
            XSSFFont font = (XSSFFont)wb.createFont();    
            font.setFontName("黑体");    
            font.setFontHeightInPoints((short) 14);//设置字体大小
            xHeardcellStyle.setFont(font);//选择需要用到的字体格式 
            
            xcellStyle = (XSSFCellStyle)wb.createCellStyle();
            xcellStyle.setBorderBottom(HSSFCellStyle.BORDER_THIN); //下边框    
            xcellStyle.setBorderLeft(HSSFCellStyle.BORDER_THIN);//左边框    
            xcellStyle.setBorderTop(HSSFCellStyle.BORDER_THIN);//上边框    
            xcellStyle.setBorderRight(HSSFCellStyle.BORDER_THIN);//右边框    
            xcellStyle.setAlignment(HSSFCellStyle.ALIGN_CENTER); // 居中    
        }
        if (data.length == 0) {
            return wb;
        }
        for (int i = 0; i < data.length; i++) {
            ...
            if (headers[i].length > 0) {
                ...
                for (int h = 0, lenH = headers[i].length; h < lenH; h++) {
                    ...
                    cell.setCellStyle(xHeardcellStyle);
                }
            }
            ...
            for (int j = 0, len = data[i].size(); j < len; j++) {
                ...
                if (obj instanceof Map) {
                    processAsMap(columns[i], row, obj,xcellStyle);
    ...
    private static void processAsMap(String[] columns, Row row, Object obj,XSSFCellStyle cellStyle) {
        Cell cell;
